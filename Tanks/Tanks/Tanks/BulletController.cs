...
namespace Tanks
{
    class BulletController
    {
        ...
        BulletView bulletColobokView = new BulletView();
        ...
        public List<Bullet> CreateBullet(Kolobok kolobok)
        {
        ...
        public PictureBox GoBullet(Point[] arrCoordinateObstacle, PictureBox pictureBox)
        {
            for (int i = 0; i < bulletKolobokList.Count; i++)
            {
                ...
                {
                    pictureBox = bulletColobokView.RemoveViewBullet(bulletKolobokList[i], pictureBox);
        ...
        public List<Bullet> GetListBullet()
        {
            return bulletKolobokList;
        }
        ...
        public bool checkCollisionBulletObstacle(Bullet bullet, Point[] arrCoordinateObstacle)
        {
            for (int i = 0; i < arrCoordinateObstacle.Length; i++)
            {
                ...
                {
                    if (collides(bullet.x, bullet.y, bullet.x + bullet.sizeX, bullet.y + bullet.sizeY + 2, arrCoordinateObstacle[i].X, arrCoordinateObstacle[i].Y - bullet.speed, arrCoordinateObstacle[i].X + 36, arrCoordinateObstacle[i].Y + 16))
                        return false;
                }
                ...
                {
                    if (collides(bullet.x, bullet.y, bullet.x + bullet.sizeX + bullet.speed, bullet.y + bullet.sizeY, arrCoordinateObstacle[i].X, arrCoordinateObstacle[i].Y, arrCoordinateObstacle[i].X + 36, arrCoordinateObstacle[i].Y + 16))
            ...
            for (int i = 0; i < tanksList.Count; i++)
            {
                for (int j = 0; j < bulletKolobokList.Count; j++)
                {
                    if (bulletKolobokList[j].direction == "UP")
                    {
                        ...
                        {
                            pictureBox = bulletColobokView.RemoveViewBullet(bulletKolobokList[j], pictureBox);
